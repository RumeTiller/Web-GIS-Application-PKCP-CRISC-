...
const CriscMap = ({ mapData }) => {
  ...
  useEffect(() => {
    ...
    mouza = new TileLayer({
      ...
      source: new TileWMS({
        url: 'http://118.179.197.118:8080/geoserver/tiller_projects/wms',
        ...
        fetch(structueUseInfo)
          ...
          .then((html) => {
            if (html.features.length > 0) {
              ...
              const shownData = `<script>
function myFunction() {
  document.getElementById("info").innerHTML = "Hello World";
}
</script>
              <div onclick="myFunction()" class='absolute right-1 top-1 text-sm hover:text-red-400 hover:cursor-pointer' >x</div>
  ...
  function legend() {
    ...
    for (i = 0; i < no_layers; i++) {
      const getImg = "http://118.179.197.118:8080/geoserver/tiller_projects/wms?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetLegendGraphic&FORMAT=image%2Fpng&LAYER=" + AllLayers.getLayers().item(i).get('description')
      ar.push(getImg);
